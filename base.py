...
class AvitoParse:
    ...
    def __get_url(self):
        ...
        self.driver.find_element(LocatorAvito.SORT[1], by="css selector").click()
        time.sleep(1)
        self.driver.find_element(LocatorAvito.BY_DATE[1], by="css selector").click()

    def __paginator(self):
        """Кнопка далее"""
        logger.info('Страница успешно загружена. Просматриваю объявления Browser 1')
        while self.count > 0:
            self.__parse_page()
            self.count -= 1
            break
    ...
    def __parse_full_page(self, url: str, data: dict) -> dict:
        ...
        try:
            self.driver.wait_for_element(LocatorAvito.TOTAL_VIEWS[1], by="css selector", timeout=1)
        except Exception:
...
def main():
    ...
    # Создаем браузер один раз для использования в AvitoParse
    try:
        with SB(headless=True,
                page_load_strategy="eager",
                block_images=True,
                skip_js_waits=True) as browser_driver:

            browser_driver.open_new_window()  # сразу открываем и вторую вкладку
            browser_driver.switch_to_window(window=0)
            browser_driver.set_wire_proxy('4RFu6K:tSC2SW@46.3.198.241:9922')
            time.sleep(0.5)
            while True:
                try:
                    urls = monitor_database()
                    # Передаем экземпляр браузера в AvitoParse
                    driver = AvitoParse(
